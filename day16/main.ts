...
import { List, Range, Record, RecordOf, Set } from "$immutable/mod.ts";
...
async function moveBeam(
	...
	const newNext = next.withMutations((tempNext) => {
		switch (grid.get(row)?.get(col)) {
			case ".": {
				...
				if (nextRow >= 0 && nextRow < grid.size && nextCol >= 0 && nextCol < grid.get(0, List()).size) {
					tempNext.push(Beam({ position: List([nextRow, nextCol]), direction: beam.direction }));
				}
				break;
			}
			case "\\": {
				...
				if (nextRow >= 0 && nextRow < grid.size && nextCol >= 0 && nextCol < grid.get(0, List()).size) {
					tempNext.push(Beam({ position: List([nextRow, nextCol]), direction: List([dirCol, dirRow]) }));
				}
				break;
			}
			case "/": {
			...
			case "-": {
				if (dirRow == 0) {
					...
					let nextCol = col - 1;
					if (nextRow >= 0 && nextRow < grid.size && nextCol >= 0 && nextCol < grid.get(0, List()).size) {
						tempNext.push(Beam({ position: List([nextRow, nextCol]), direction: List([0, -1]) }));
					}
					nextCol = col + 1;
					if (nextRow >= 0 && nextRow < grid.size && nextCol >= 0 && nextCol < grid.get(0, List()).size) {
						tempNext.push(Beam({ position: List([nextRow, nextCol]), direction: List([0, 1]) }));
			...
			case "|": {
				if (dirCol == 0) {
					...
					const nextCol = col;
					if (nextRow >= 0 && nextRow < grid.size && nextCol >= 0 && nextCol < grid.get(0, List()).size) {
						tempNext.push(Beam({ position: List([nextRow, nextCol]), direction: List([-1, 0]) }));
					}
					nextRow = row + 1;
					if (nextRow >= 0 && nextRow < grid.size && nextCol >= 0 && nextCol < grid.get(0, List()).size) {
						tempNext.push(Beam({ position: List([nextRow, nextCol]), direction: List([1, 0]) }));
...
async function run2(inputPath: string): Promise<number> {
	...
	const bottom = Range(0, width).map((col) => moveBeam(Beam({position: List([height - 1, col]), direction: List([-1, 0])}), grid))
	const left = Range(0, height).map((row) => moveBeam(Beam({position: List([row, 0]), direction: List([0, 1])}), grid))
	const right = Range(0, height).map((row) => moveBeam(Beam({position: List([row, width - 1]), direction: List([0, -1])}), grid))
	const all = top.concat(bottom, left, right)
	const max = all.reduce(async (currMaxPromise, nextPromise, idx) => {
		console.log(idx)
		await delay(10)
		const currMax = await currMaxPromise
		const next = await nextPromise
		const nextPos = next.map(R.prop('position'))
		return Math.max(currMax, nextPos.size)
